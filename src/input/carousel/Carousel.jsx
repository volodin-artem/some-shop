...
class Carousel extends React.Component {
  constructor(props) {
    super(props);
    this.state = { right: 0, transition: "", oldPosition: 0, zero: 0 };
    this.maxWidth = 0;
  ...
  onMouseMove(e){
    ...
    if((x > this.maxWidth - this.props.offset) || (x <= 0 && this.state.right <= 0)) return;
  ...
  render() {
    ...
    const { right } = this.state;
    this.maxWidth = this.props.items.length * this.props.offset;
    ...
    return (
      <div className="banner" style={style} onMouseDown={ this.onMouseDown }>
        ...
        <div className="carousel" style={{ right: right, transition: this.state.transition, width: this.maxWidth }}>
          { this.props.items }
        </div>
